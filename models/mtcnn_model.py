...
def cls_ohem(cls_prob, label):
    ...
    cls_prob_reshape = tf.reshape(cls_prob, [num_cls_prob, -1])  # 将预测值reshape成一列
    label_int = tf.cast(label_filter_invalid, tf.int32)  # 数据类型转换
    # get the number of rows of class_prob
    num_row = tf.to_int32(cls_prob.get_shape()[0])  # 得到预测值的行数，即batch_size
    # row = [0,2,4.....]
    row = tf.range(num_row) * 2
    indices_ = row + label_int  # 得到的索引位置
    label_prob = tf.squeeze(tf.gather(cls_prob_reshape, indices_))  # tf.gather根据对应索引取值，这里是找到正例的得分
    loss = -tf.log(label_prob + 1e-10)  # 计算对应位置的损失
    ...
    loss = loss * valid_inds  # 正例样本的损失
    loss, _ = tf.nn.top_k(loss, k=keep_num)  # 选取前keep_num个损失
...
def bbox_ohem(bbox_pred, bbox_target, label):
    ...
    valid_inds = tf.where(tf.equal(tf.abs(label), 1), ones_index, zeros_index)  # 找出有回归边框的索引位置
    # (batch,)
    # calculate square sum
    square_error = tf.square(bbox_pred - bbox_target)  # shape is (batch_size,4)
    square_error = tf.reduce_sum(square_error, axis=1)  # 对行求和，shape is (batch_size,)
    # keep_num scalar
    num_valid = tf.reduce_sum(valid_inds)  # 求所有正例的个数
    # keep_num = tf.cast(num_valid*num_keep_radio,dtype=tf.int32)
    # count the number of pos and part examples
    keep_num = tf.cast(num_valid, dtype=tf.int32)
    # keep valid index square_error
    square_error = square_error * valid_inds  # 正例有损失，负例无损失
...
def landmark_ohem(landmark_pred, landmark_target, label):
    ...
    valid_inds = tf.where(tf.equal(label, -2), ones, zeros)  # 将label为-2的索引设置为1，其他的设置为0
...
# construct Pnet
# label:batch
# label表示是否是人脸，bbox_target代表矩形框的位置，landmark_target代表关键点的位置
def P_Net(inputs, label=None, bbox_target=None, landmark_target=None, training=True):
    ...
    with slim.arg_scope([slim.conv2d],
        ...
        if training:
            # batch*2
            # calculate classification loss，shape is batch*H*W*2
            cls_prob = tf.squeeze(conv4_1, [1, 2], name='cls_prob')  # 训练阶段conv4_1 shape is (batch_size,1,1,2)
            cls_loss = cls_ohem(cls_prob, label)  # label shape is (batch_size,);cls_prob shape is (batch_size,2)
            # batch
            # cal bounding box error, squared sum error
            bbox_pred = tf.squeeze(bbox_pred, [1, 2], name='bbox_pred')  # 训练阶段bbox_pred shape is (batch_size,1,1,4)
            bbox_loss = bbox_ohem(bbox_pred, bbox_target, label)
            # batch*10  训练阶段landmark_pred shape is (batch_size,1,1,10)
