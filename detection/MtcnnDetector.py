...
sys.path.append("../")
from models.MTCNN_config import config
...
class MtcnnDetector(object):
    ...
    def generate_bbox(self, cls_map, reg, scale, threshold):
        ...
        t_index = np.where(cls_map > threshold)  # 返回的是索引
        ...
        boundingbox = np.vstack([np.round((stride * t_index[1]) / scale),  # 左上角x的坐标
                                 np.round((stride * t_index[0]) / scale),  # 左上角y的坐标
                                 np.round((stride * t_index[1] + cellsize) / scale),  # 右下角x的坐标
                                 np.round((stride * t_index[0] + cellsize) / scale),  # 右下角y的坐标
                                 score,
                                 reg])
    ...
    def processed_image(self, img, scale):
        ...
        new_height = int(height * scale)  # resize new height
        new_width = int(width * scale)  # resize new width
        new_dim = (new_width, new_height)
        img_resized = cv2.resize(img, new_dim, interpolation=cv2.INTER_LINEAR)  # resize image
    ...
    def detect_pnet(self, im):
        ...
        # resize image using current_scale
        ...
        # refine the boxes
        boxes_c = np.vstack([all_boxes[:, 0] + all_boxes[:, 5] * bbw,  # 左上角x的值+x1_offset×box的宽
                             all_boxes[:, 1] + all_boxes[:, 6] * bbh,  # 右上角y的值+y1_offset×box的高
                             all_boxes[:, 2] + all_boxes[:, 7] * bbw,  # 右下角x的值+x2_offset×box的宽
                             all_boxes[:, 3] + all_boxes[:, 8] * bbh,  # 右下角y的值+yy_offset×box的高
                             all_boxes[:, 4]])
        boxes_c = boxes_c.T
    ...
    def detect_rnet(self, im, dets):
        ...
        cls_scores, reg, _ = self.rnet_detector.predict(cropped_ims)  # 获得概率值、偏移系数值
